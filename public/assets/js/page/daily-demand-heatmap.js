...
const siteHeader = createApp({
    setup() {
        const highlight = ref("daily-demand-heatmap");
...
const setup = {
    ...
    setup() {
        ...
        const onSuccess = (api_response) => {
            ...
            cleanedChartData.value = chartData.value.map(entry => {
                ...
                if (entry.data < currentDateString || entry.data === currentDateString) {
                    ...
                    // imaemsUI.log("cleanedValue: ", cleanedValue);
                    ...
                    // imaemsUI.log("filteredData: ", filteredData);
            ...
            const newDateArr = cleanedChartData.value.map(entry => {
                const max = Math.max(...entry.value);
            ...
            max_date.value = `${overallMaxDate}(${overallMaxValue} 瓩)`;
        ...
        const { querySubmit, isdisabled } = useApiRequest('https://api.seochibao.com/api/daily-demand-heatmap', validate, queryData, onSuccess, onFailure, member_level);
        const handleSubmit = () => {
            isResult.value = 0;
            // 觸發第一次請求
            querySubmit();
        ...
        const downloadFile = async () => {
            ...
            const apiEndpoint = 'https://api.seochibao.com/api/download/daily-demand-heatmap';
...
const DailyDemandHeatmapApp = createApp(setup);
DailyDemandHeatmapApp.config.compilerOptions.isCustomElement = (tag) => {
    return tag.startsWith('module-');
}
DailyDemandHeatmapApp.mount("#DailyDemandHeatmapApp");
